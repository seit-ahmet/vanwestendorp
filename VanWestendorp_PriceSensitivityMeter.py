...
def validate(df, price_cols):
    ...
    new_size = df.shape[0]
    print(
        f"Total data set contains {original_size} cases, {new_size} cases were kept (transitive price preferences).\n")
    return df
...
def cdf_table(df, price_cols, interpolate=True):
    ...
    '''
    df.rename(columns={price_cols[0]: "Too Cheap", price_cols[1]: "Cheap", price_cols[2]: "Expensive",
                       price_cols[3]: "Too Expensive"})
    ...
    if interpolate:
        low = cdfs.Price.min()
        high = cdfs.Price.max()
        cdfs = pd.merge(pd.DataFrame(list(np.arange(low, high, 0.01)), columns=['Price']), cdfs,
                        how='outer').sort_values('Price')
...
def plot_function(cdfs,
    ...
    trace1 = go.Scatter(
        x=cdfs.Price.values,
        y=cdfs[var].values,
        text=[f"{var}<br>Price: ${price:.2f}<br>Participants: {val * 100:.2f}%" for (price, val) in
              zip(cdfs.Price.values, cdfs[var].values)],
        mode='lines',
        opacity=0.8,
        marker={
            'size': marker_size,
    ...
    point1 = go.Scatter(
        ...
        y=[PMC_height],
        text=[
            f"Point of Marginal Cheapness: ${Point_of_Marginal_Cheapness:.2f}<br>Participants: {PMC_height * 100:.2f}%"],
        mode='markers',
        opacity=1,
        marker={
            'size': 7,
    ...
    point2 = go.Scatter(
        ...
        y=[PME_height],
        text=[
            f"Point of Marginal Expensiveness: ${Point_of_Marginal_Expensiveness:.2f}<br>Participants: {PME_height * 100:.2f}%"],
        mode='markers',
        opacity=1,
        marker={
            'size': 7,
...
def results(df, price_cols, plot=True, plot_title=""):
    ...
    cdfs.iloc[np.argwhere(np.diff(np.sign(cdfs['Too Cheap'] - cdfs['Not Cheap']))).flatten() + 1]['Price'].values[0]
    Point_of_Marginal_Expensiveness = \
    cdfs.iloc[np.argwhere(np.diff(np.sign(cdfs['Too Expensive'] - cdfs['Not Expensive']))).flatten() + 1][
        'Price'].values[0]
    Indifference_Price_Point = \
    cdfs.iloc[np.argwhere(np.diff(np.sign(cdfs['Not Cheap'] - cdfs['Not Expensive']))).flatten() + 1]['Price'].values[0]
    Optimal_Price_Point = \
    cdfs.iloc[np.argwhere(np.diff(np.sign(cdfs['Too Expensive'] - cdfs['Too Cheap']))).flatten() + 1]['Price'].values[0]

    # For the plot
    PMC_height = cdfs.iloc[np.argwhere(np.diff(np.sign(cdfs['Too Cheap'] - cdfs['Not Cheap']))).flatten() + 1][
        ['Too Cheap', 'Not Cheap']].mean(axis=1).values[0]
    PME_height = cdfs.iloc[np.argwhere(np.diff(np.sign(cdfs['Too Expensive'] - cdfs['Not Expensive']))).flatten() + 1][
        ['Too Expensive', 'Not Expensive']].mean(axis=1).values[0]
    IPP_height = cdfs.iloc[np.argwhere(np.diff(np.sign(cdfs['Not Cheap'] - cdfs['Not Expensive']))).flatten() + 1][
        ['Not Cheap', 'Not Expensive']].mean(axis=1).values[0]
    OPP_height = cdfs.iloc[np.argwhere(np.diff(np.sign(cdfs['Too Expensive'] - cdfs['Too Cheap']))).flatten() + 1][
        ['Too Expensive', 'Too Cheap']].mean(axis=1).values[0]
